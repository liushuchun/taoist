...
namespace cnn{
    ...
    struct Dim{
        ...
        // Dim(unsigned int m,unisinged int n):nd(2),bd(1){d[0]=m;d[1]=n;}
        ...
        inline unsigned int size(unsigned int i){return (*this)[i];}
        inline Dim transpose() const{
            if(nd==1){return Dim({1,d[0]},bd);}
            else if(nd==2){return Dim({d[1],d[0]},bd);}
            throw std::invalid_argument("Cant transpose Dim object with more thant 2 dims");
        }


        unsigned  int nd;
        unsigned int bd;
        unsinged int d[CNN_MAX_TENSOR_DIM];
